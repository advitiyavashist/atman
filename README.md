...
Runtime commands such as `watch`, `spawn`, `hooks`, `ui`, `quickstart`, and
`--version` come from root `tickets.py`. `pip install -e .` and the wheel
console script dispatch to that same file (`ticket_board.entry`). `./install.sh`
is still the live-release symlink path.

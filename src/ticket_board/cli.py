...
def main():
    """Console entry: same implementation as root tickets.py (T-1380).

    Helpers in this module remain for dual-copy tests and library imports.
    Packaged ``atm``/``tickets`` scripts and ``python -m ticket_board`` all
    call tickets.main so brew, pip, shims, and checkouts cannot diverge.
    """
    try:

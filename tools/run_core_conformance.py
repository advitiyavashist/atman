    ...
    if path.suffix in {".json", ".jsonl"} or path.name.endswith(".jsonl"):
        ...
        if path.suffix == ".json":
            try:
                ...
                # STORE-001 leaves deliberate corrupt committed records; snapshot
                # must still be deterministic without raising.
        ...
        for line in text.splitlines():
            ...
            try:
                ...
                lines.append(_normalize_stream(line, variables))
                continue
            encoded = json.dumps(payload, sort_keys=True, separators=(",", ":"))
...
def _board_tree_snapshot(
    ...
    # Agent checkins embed git dirty/sha and wall-clock fields; never hash contents.
    # MASTER.md / briefs also embed live timestamps that survive ISO redaction poorly.
    names_only = tuple(names_only) + (
        "agents/",
        "MASTER.md",
        "CONTEXT.md",
        "briefs/",
...
def _base_env(variables: dict[str, str], agent: str) -> dict[str, str]:
    env = dict(os.environ)
    for name in (
        "GIT_DIR", "GIT_COMMON_DIR", "GIT_WORK_TREE", "PYTHONPATH",
        "TICKET_SEAT", "TICKETS_WATCH_PINNED", "TICKETS_RUN_ID", "TICKETS_RUN_NO",
        "TICKET_SESSION_ID",
    ):
        env.pop(name, None)
    env.update(
...
def _prepare(case: dict[str, Any], root: Path, suite_dir: Path) -> dict[str, str]:
    ...
    # Isolate machine-dependent dirty counts: fixture board + home are never
    # part of the candidate under test (T-805 / T-817).
    (root / ".gitignore").write_text(".fixture-board/\nhome/\n")
    variables = {
        "ROOT": str(root),
        "HOME": str(home),
        "BOARD": str(board),
                ...
                plan = [
                    {
                        ...
                        "deps": [],
                        "cause": "conformance load",
                        "change": "seed ready tickets",
                        "proof": "warm board p95",
                    }

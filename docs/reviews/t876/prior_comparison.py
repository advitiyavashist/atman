...
REPAIRED = Path(__file__).resolve().parents[3]
PRIOR = Path('/Users/kavana/Downloads/atman/.worktrees/atman-identity-finalverify-codex-0913')
scratch = Path(tempfile.mkdtemp(prefix='t847-prior-comparison-', dir='/private/tmp'))
print('SCRATCH', scratch, flush=True)
for name, root in [('prior-feca2f5', PRIOR), ('candidate-2a942f0', REPAIRED)]:
    repo = scratch / name
    repo.mkdir()
    board = repo / '.tickets'

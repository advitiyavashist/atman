...
- Protected checkouts come from `TICKET_BOARD_FORBIDDEN_ROOTS`, an
  `os.pathsep`-separated list of absolute directories. Setting it replaces the
  default (no implicit protected roots). Setting it to the literal `NONE`
  disables protection entirely. Nonempty overrides must contain existing,
...
- Add every live fleet checkout you must never enroll to
  `TICKET_BOARD_FORBIDDEN_ROOTS`. Clones outside that list are separate
  repositories; protect them explicitly if they must not be enrolled.

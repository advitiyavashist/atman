...
The bus is `tickets msg` / `tickets inbox`, not Gemini chat. Persistent seats
need `join --persistent` plus these hooks. Claude→agy and agy→Cursor must
land in inbox the same way Claude→Claude does.

...
```sh
atm join "$TICKET_AGENT" --harness agy --roles backend --persist \
...
```sh
atm spawn NAME --harness agy --persist --roles backend
...
says `supervised (...)` rather than `woken`. `atm join --persistent` will
tell you the same. Evidence: docs/wake-recipients.md.

## 2. Hooks

```sh
atm hooks agy --agent "$TICKET_AGENT"
```

Writes project `.agents/hooks.json`:

- `PreInvocation` → `atm hook-run --event agy-inbox` (inject unread mail)
- `Stop` → `atm hook-run --event agy-stop` (`block` mapped to `continue`
...
The bus is `atm msg` / `atm inbox`. If native inject has no live

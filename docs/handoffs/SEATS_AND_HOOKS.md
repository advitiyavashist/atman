...
Repos: `$HOME/Downloads/atman` -> `advitiyavashist/atman`; `$HOME/Downloads/steer` -> `advitiyavashist/steer`. Shared board for both: `$HOME/Downloads/steer/.tickets`. Never initialize a second board or edit its JSON. Ticket repo evidence and deliverable origin matter more than folder names.
...
```sh
export TICKETS_DIR=$HOME/Downloads/steer/.tickets

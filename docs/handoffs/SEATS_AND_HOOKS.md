...
Repos: `$HOME/Downloads/atman` -> `advitiyavashist/atman`; `$HOME/Downloads/steer` -> `advitiyavashist/steer`. One shared board for both: find it with `atm where`; if a repo resolves elsewhere, link it once with `atm board-link <shared .tickets>`. Never initialize a second board or edit its JSON. Ticket repo evidence and deliverable origin matter more than folder names.
...
```sh
# the board comes from `atm where` (repo linked via `atm board-link`); no TICKETS_DIR export needed

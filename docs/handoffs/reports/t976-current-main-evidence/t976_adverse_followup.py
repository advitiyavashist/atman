...
def failed_check():
 ...
 atm(repo,'reject','T-001','--sha',sha,'--reason','Check failed: expected rows 3, got 999',agent='bob')
...
def transfer():
 ...
 atm(repo,'join','carol','--roles','backend','--harness','codex',agent='carol');before=ticket(repo);atm(repo,'assign','T-001','--owner','carol');after=ticket(repo)
 assert after['owner']=='carol' and after['owner_generation']==before['owner_generation']+1
...
for name,fn in [('failed-check',failed_check),('interchangeability-metadata',transfer)]:record(name,fn)

...
checking qwen: harness=custom cmd=/tmp/byoa2/echo-agent.sh {prompt_file} {cwd} {agent}
  cmd:      /tmp/byoa2/echo-agent.sh /var/folders/.../tickets-prompt-qwen-ldpjocfa.txt $TMPDIR/byoa2/repo qwen
  exit:     0
  latency:  0.8s
  replied:  no 'OK' in the first 400 chars
  output:   prompt is        8 bytes; cwd $TMPDIR/byoa2/repo; agent qwen
qwen: harness OK
```
...
qwen           pid 64905 custom    -        ok 0.8s      0m ago   $TMPDIR/byoa2/repo/.worktrees/qwen
```

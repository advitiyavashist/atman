...
Two delivery modes. Do not mix them on one machine without knowing which one
`PATH` hits first.

**A. Clone on PATH (development, this checkout)**
...
**B. Pinned live-release snapshot (shared CLI)**
...
end, and the plan examples in older docs all omit the three fields.
...
**`done` and `review` refuse from `main` and refuse a dirty tree.** Work on
the agent's branch. Notes are required.

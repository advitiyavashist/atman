...
| — no field, `additionalProperties: false` — | `priority`, `epic`, `sprint`, `needs` | **zero hits** for any of these four in `openapi.yaml`. Sprint/epic grouping and `tickets next`'s priority-1-vs-routine ordering are unrepresentable in the contract as frozen — every ticket routed by priority today (e.g. this one) has no server-side equivalent. |
...
| `owner`: `AgentId` (`^agt_[0-9a-z]{8,32}$`), nullable | `owner`: free string (`claude-fable`, `gpt-codex`, `sonnet-sdk`, ...) | every real owner value on this board fails the contract pattern. See §1 Identity. |
...
| `body` (≤4000 chars) | `text` | name differs; contract caps length, CLI does not (T-213's legacy-import audit found 2 real notes over 4000 chars and had to invent a split-and-rejoin scheme to archive them). |
...
- `AgentId` pattern is `^agt_[0-9a-z]{8,32}$` (`openapi.yaml:165-167`). Every
  CLI owner in production is a free-form name (`claude-fable`, `sonnet-qa`,
  `cursor-2`, ...) — **0 of the ~35 real agent names on this board match the
...
- Contract `Ticket.outcome` is required to create and `TicketCreateRequest`

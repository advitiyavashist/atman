"""T-016: `plan` resolved dependency `key`s only within the single JSON
payload of that one invocation. A second `plan` call that tried to depend
on an earlier plan's key failed with "no such key or id" -- worded exactly
like the ticket did not exist, when the real problem was that the key was
never persisted anywhere to look it up. Ticket ids always worked as the
workaround; this closes the gap so the key does too.
...
def test_within_one_plan_call_the_key_still_resolves_as_before(repo):
    """Same-invocation resolution (the thing that already worked) must not
    regress now that cross-invocation lookup was added."""
    r = plan(repo, {"tickets": [
...
def test_an_unresolvable_reference_still_fails_clearly_and_rolls_back(repo):
    ...
    # nothing was left behind from the failed attempt
    after = {t["id"] for t in list_json(repo)}
    assert after == before


def test_a_ticket_id_still_works_directly_as_a_dep_workaround(repo):
    """The documented workaround (use the id, not the key) must keep
    working -- this ticket adds a second path, it does not remove the
    first."""

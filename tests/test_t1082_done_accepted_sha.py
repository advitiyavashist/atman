"""T-1082: `atm done` without --artifact records the accepted sha, not cwd HEAD.

Two-copy: tickets.py and src/ticket_board/cli.py.
Throwaway board. Coordinator close from trunk after accept must not hand
main@HEAD to the successor.
"""
...
def test_done_pin_state_defaults_to_accepted_sha():
    ...
    assert warn and "accepted_sha" in warn and later[:12] in warn
    ...
    honoured, _ = work_view.done_pin_state(t, g, honor_cwd=True)
    assert honoured["sha_full"] == later
...
def test_done_without_accept_still_records_cwd(tool, board):
    ...
    assert "accepted_sha" not in (done.stderr or "")

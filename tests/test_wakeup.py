...
def run(board, *args, agent="", stdin="", env=None, cwd=None):
    ...
    # be sitting in (e.g. CLAUDE_CODE_SESSION_ID from an outer Claude Code
    # session) -- left in place, every subprocess this file launches would
    # share one identical session key.
    #
    ...
    # all collide on that single shared file -- the exact sideways bleed
    # T-018 exists to prevent, just re-triggered by the test doing what no
    # single real harness session would.
    #

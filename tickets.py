...
def git(*args, **kw):
    ...
    cwd = kw.pop("cwd", None)
    if kw:
        raise TypeError("git() got unexpected keyword arguments %s" % sorted(kw))
    try:
...
def git_state(cwd=None):
    """Branch, short sha, dirty-file count, and whether this is the main worktree.

    `cwd` is the tree to describe (default: the process cwd). All four parts of
    the answer -- branch, sha, repo identity and dirty count -- come from that
    one tree, so the pin they form is internally consistent (T-272).
    """
    top = git("rev-parse", "--show-toplevel", cwd=cwd)
    if not top:
        return None
    branch = git("rev-parse", "--abbrev-ref", "HEAD", cwd=cwd) or "?"
    sha = git("rev-parse", "--short", "HEAD", cwd=cwd) or "?"
    dirty = git("status", "--porcelain", cwd=cwd)
    common = git("rev-parse", "--git-common-dir", cwd=cwd) or ""
    gitdir = git("rev-parse", "--git-dir", cwd=cwd) or ""
...
def cmd_sync(a, board):
    ...
    r = subprocess.run(["git", "merge", "--no-edit", "-m", "Sync %s into %s" % (trunk, g["branch"]), trunk],
                       cwd=art, capture_output=True, text=True)
    if r.returncode == 0:

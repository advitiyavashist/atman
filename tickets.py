...
def cmd_note(a, board):
    ...
    as the owner's, so nothing in the note history could ever reveal the
    second lane. T-956: whoami() here drifted from the module rule that
    ...
    to identity. The on-disk `by` is the write-path truth (`atm show`).
    """
    t = load(board, a.id)
...
_KNOWLEDGE_MARKER = "...(knowledge budget reached; run `atm knowledge query ...`)"
...
def knowledge_context(board, owner, extra="", max_chars=None, meta=None):
    ...
    if errors:
        ...
        return _finish("Knowledge graph invalid; run `atm knowledge validate`.")

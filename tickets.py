...
try:
    ...
    from ticket_board import tickets_harness as _tickets_harness
_tickets_harness.attach(globals())
harness_auth_probe = _tickets_harness.harness_auth_probe
_print_auth_result = _tickets_harness._print_auth_result
cmd_harness_auth = _tickets_harness.cmd_harness_auth
_harness_check_label = _tickets_harness._harness_check_label
harness_probe = _tickets_harness.harness_probe
usage_probe_is_spawn = _tickets_harness.usage_probe_is_spawn
catalog_usage_argv = _tickets_harness.catalog_usage_argv
_usage_key_norm = _tickets_harness._usage_key_norm
_usage_field_str = _tickets_harness._usage_field_str
_usage_fields_from_mapping = _tickets_harness._usage_fields_from_mapping
parse_usage_remaining_reset = _tickets_harness.parse_usage_remaining_reset
_run_usage_probe = _tickets_harness._run_usage_probe
probe_catalog_usage = _tickets_harness.probe_catalog_usage
attach_catalog_usage = _tickets_harness.attach_catalog_usage
_fmt_usage_field = _tickets_harness._fmt_usage_field
probe_integration_catalog = _tickets_harness.probe_integration_catalog
_print_catalog_usage_line = _tickets_harness._print_catalog_usage_line
cmd_harness_usage = _tickets_harness.cmd_harness_usage
cmd_harness_available = _tickets_harness.cmd_harness_available
cmd_harness = _tickets_harness.cmd_harness

# T-1050: watch/spawn topic lives in tickets_watch.py; names stay on the facade.
_here_watch = os.path.dirname(os.path.realpath(__file__))
if _here_watch not in sys.path:
    sys.path.insert(0, _here_watch)
try:
    import tickets_watch as _tickets_watch
except ImportError:
    from ticket_board import tickets_watch as _tickets_watch
_tickets_watch.attach(globals())
_watch_run_capped = _tickets_watch._watch_run_capped
_t427_release_dir = _tickets_watch._t427_release_dir
_t427_shim_tickets = _tickets_watch._t427_shim_tickets
_t427_verified_sha = _tickets_watch._t427_verified_sha
_t427_idle_shim_path = _tickets_watch._t427_idle_shim_path
watch_idle_reexec = _tickets_watch.watch_idle_reexec
_reexec_watch_if_unpinned = _tickets_watch._reexec_watch_if_unpinned
cmd_watch = _tickets_watch.cmd_watch
_spawn_stop = _tickets_watch._spawn_stop
_stop_requested = _tickets_watch._stop_requested
cmd_spawn = _tickets_watch.cmd_spawn
_resolve_spawn_target = _tickets_watch._resolve_spawn_target
_spawn_git_root = _tickets_watch._spawn_git_root

# T-1050: review/merge topic lives in tickets_merge.py; names stay on the facade.
_here_merge = os.path.dirname(os.path.realpath(__file__))
if _here_merge not in sys.path:
    sys.path.insert(0, _here_merge)
try:
    import tickets_merge as _tickets_merge
except ImportError:
    from ticket_board import tickets_merge as _tickets_merge
_tickets_merge.attach(globals())
cmd_review = _tickets_merge.cmd_review
cmd_accept = _tickets_merge.cmd_accept
cmd_reject = _tickets_merge.cmd_reject
MERGE_CONFIG_DEFAULT = _tickets_merge.MERGE_CONFIG_DEFAULT
merge_config = _tickets_merge.merge_config
parse_review_sha = _tickets_merge.parse_review_sha
require_integrator = _tickets_merge.require_integrator
IntegrationLock = _tickets_merge.IntegrationLock
_order_shas_by_ancestry = _tickets_merge._order_shas_by_ancestry
cmd_merge = _tickets_merge.cmd_merge
...
def cmd_ui(a, board):
    ...
    class H(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            try:
                payload = _ui_read_json_body(self)
                # T-1104 decides WHO the post is from (the operator, never a
                # seat); T-1106 decides that it wakes its recipients the way
                # `atm msg` does. Both, or the app is either a forgery or a
                # message nobody is woken for.
...
# Late facade names (e.g. _release_commit) are defined after the first
# tickets_watch.attach(); re-bind so cmd_watch sees them.
_tickets_watch.attach(globals())
